...
#         self.conv1_1 = DepthWiseConv2d(feature, 3, 1, True)
...
class ICSPFPN(nn.Module):
    def __init__(self, feature_map: List[int], feature: int):
        ...
        self.icsp_blcok4 = TestModule(feature, 4, 2)
        self.icsp_blcok5 = TestModule(feature, 4, 2)
        self.icsp_blcok6 = TestModule(feature, 4, 2)
        self.icsp_blcok7 = TestModule(feature, 4, 2)
...
class SEBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x1 = self.Gap(x)  # torch.Size([1, 128, 1, 1])
        x1 = self.pw_conv1(x1)
        x1 = self.act(x1)
        x1 = self.pw_conv2(x1)
        x1 = self.sigmoid(x1)
