...
class FRFCOS(nn.Module):
    ...
    def __init__(self, feature_map:List[int],
        ...
        self.refine = Refine_Module(feature_lv, 512)
...
class ICSPFPN(nn.Module):
    def __init__(self, feature_map: List[int], feature_lv: List[int]):
        ...
        self.tf1 = PointWiseConv(feature_map[2],feature_lv[2])
        self.tf2 = PointWiseConv(feature_map[1],feature_lv[1])
        self.tf3 = PointWiseConv(feature_map[0],feature_lv[0])
...
class Refine_Module(nn.Module):
    def __init__(self, feature_lv: List[int], feature:int):
        ...
        self.pw_conv2 = PointWiseConv(feature_lv[2]+feature_lv[0], feature, bs = True)
        self.pw_conv3 = PointWiseConv(feature, feature, bs = True)
        self.pw_conv4 = PointWiseConv(feature, feature_lv[1], bs = True)
        self.pw_conv5 = PointWiseConv(feature, feature_lv[0], bs = True)
...
class ClassificationSub(nn.Module):
    ...
    def forward(self, x:torch.Tensor) -> torch.Tensor:
        x = self.relu(self.bn(self.cls_c1(x)))
        x = self.relu(self.bn(self.cls_c2(x)))
        cls = self.cls(x)
        return cls
...
class RegressionSub(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.relu(self.bn(self.reg_c1(x)))
        x = self.relu(self.bn(self.reg_c2(x)))
        reg = self.reg(x)
