...
class COCOGenerator(CocoDetection):
    ...
    def __init__(self, imgs_path, anno_path, resize_size=[512, 512]):
...
def evaluate_coco(generator, model, threshold=0.05):
    ...
    for index in tqdm(range(len(generator))):
        ...
        scores, labels, box = head(out)
        boxes = Clip(img.unsqueeze(dim=0).cuda(), box)
...
if __name__ == "__main__":
    ...
    # model = od.HISFcos.HalfInvertedStageFCOS([512, 1024, 2048], 80, 256).to(device)
